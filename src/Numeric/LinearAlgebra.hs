...
asColumn (V r xs) = M r 1 (I.map (:[]) xs)
...
scale x (V n ys) = V n (I.scale x ys)
...
vXm (V r xs) (M _ c yss) = V c (I.vXm r c xs yss)
...
mXm (M i j xss) (M _ k yss) = M i k (I.mXm i j k xss yss)
