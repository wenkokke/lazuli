...
import Numeric.LinearAlgebra.Internal (List, length)
...
asColumn (V r xs) = M r 1 (Internal.map (:[]) xs)
...
ap (V n fs) (V _ xs) = V n (Internal.ap fs xs)
